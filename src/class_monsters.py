...
class Skeleton(Monster):
    ...
    Скелеты иммуны к колющему и режущему оружию, но получают дополнительный урон от ударного."""
    ...
    def get_weakness(self, weapon:Weapon) -> float:
        ...
        Коэффициент зависит от типа оружия: скелеты получают 
        дополнительный урон от ударного оружия и почти не получают 
        урона от колющего и рубящего. Для пустого или неизвестного 
        типа оружия возвращается нейтральный множитель 1."""    
        
        return Skeleton._weapon_type_weaknesses.get(weapon.weapon_type, 1)
